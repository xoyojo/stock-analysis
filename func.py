...
def day2csv(source_dir, file_name, target_dir):
    ...
    # 打开目标文件，后缀名为CSV
    target_path = (
        target_dir + os.sep + file_name[2:-4] + ".csv"
    )  # 目标文件包含文件名的路径
    ...
    if not os.path.isfile(target_path):
        ...
        if row_number == 0:  # 如果文件出错是0的特殊情况
            ...
            row_number = (
                row_number - 1
            )  # 由于pandas的dataFrame格式索引从0开始，为下面for循环需要减1
            begin = row_number * 32
...
def get_lastest_stocklist():
    ...
    print(
        f"优选通达信行情服务器 也可直接更改为优选好的 {{'ip': '123.125.108.24', 'port': 7709}}"
    )
    ...
    data = data.reindex(
        columns=["sse", "code", "name", "pre_close", "volunit", "decimal_point"]
    )
    ...
    sh_start_num = data[
        (data["sse"] == "sh") & (data["code"] == "600000")
    ].index.tolist()[0]
    sh_end_num = data[
        (data["sse"] == "sh") & (data["code"] == "706070")
    ].index.tolist()[0]
    sz00_start_num = data[
        (data["sse"] == "sz") & (data["code"] == "000001")
    ].index.tolist()[0]
    sz00_end_num = data[
        (data["sse"] == "sz") & (data["code"] == "100303")
    ].index.tolist()[0]
    sz30_start_num = data[
        (data["sse"] == "sz") & (data["code"] == "300001")
    ].index.tolist()[0]
    sz30_end_num = data[
        (data["sse"] == "sz") & (data["code"] == "395001")
    ].index.tolist()[0]
...
class ManyThreadDownload:
    ...
    def run(self, url, name):
        ...
        self.fd.truncate(
            self.total
        )  # 建一个和下载文件一样大的文件,不是必须的,stream=True时会用到
        self.fd.close()
        ...
        for i in range(self.num):
            t = threading.Thread(
                target=self.download, name="th-" + str(i), kwargs={"ts_queue": ts_queue}
            )
            t.setDaemon(True)
...
def readall_local_cwfile():
    ...
    for cwfile in cwfile_list:
        if os.path.getsize(ucfg.tdx["csv_cw"] + os.sep + cwfile) != 0:
            dict[cwfile[4:-4]] = pd.read_pickle(
                ucfg.tdx["csv_cw"] + os.sep + cwfile, compression=None
            )
...
def make_fq(code, df_code, df_gbbq, df_cw="", start_date="", end_date="", fqtype="qfq"):
    ...
    if "adj" in df_code.columns.to_list():
        if True in df_code["adj"].isna().to_list():
            first_index = np.where(df_code.isna())[0][
                0
            ]  # 有NaN值，设为第一个NaN值所在的行
        else:
            return ""
    ...
    df_cqcx = df_cqcx.assign(
        date=pd.to_datetime(df_cqcx["权息日"], format="%Y%m%d")
    )  # 添加date列，设置为datetime64[ns]格式
    df_cqcx.set_index(
        "date", drop=True, inplace=True
    )  # 设置权息日为索引  (字符串表示的日期 "19910101")
    df_cqcx["category"] = 1.0  # 添加category列
    df_gbbq = df_gbbq.assign(
        date=pd.to_datetime(df_gbbq["权息日"], format="%Y%m%d")
    )  # 添加date列，设置为datetime64[ns]格式
    df_gbbq.set_index(
        "date", drop=True, inplace=True
    )  # 设置权息日为索引  (字符串表示的日期 "19910101")
    if (
        ...
        cqcx_lastest_date = df_cqcx.index[-1].strftime(
            "%Y-%m-%d"
        )  # 提取最新的除权除息日
    else:
    ...
    if (
        cqcx_lastest_date not in df_code.loc[first_index:, "date"].to_list()
        and not flag_newstock
    ):
        if "adj" in df_code.columns.to_list():
            ...
            df_code_original.dropna(
                how="any", inplace=True
            )  # 丢掉缺失数据的行，之后直接append新数据就行。比merge简单。
            df_code_original["date"] = pd.to_datetime(
                df_code_original["date"], format="%Y-%m-%d"
            )  # 转为时间格式
            df_code_original.set_index(
                "date", drop=True, inplace=True
    ...
    data["preclose"] = (
        ...
        - data["分红-前流通盘"]
        + data["配股-后总股本"] * data["配股价-前总股本"]
    ) / (10 + data["配股-后总股本"] + data["送转股-后流通盘"])
    ...
    for (
        ...
        if (
            df_ltg.index[0].strftime("%Y%m%d")
            <= cw_date
            <= df_ltg.index[-1].strftime("%Y%m%d")
            and len(cw_dict[cw_date]) > 0
        ):
            if (cw_dict[cw_date][0] == code).any():
                # 获取目前股票所在行的索引值，具有唯一性，所以直接[0]
                code_df_index = cw_dict[cw_date][
                    cw_dict[cw_date][0] == code
                ].index.to_list()[0]
    ...
    )  # 重置索引行，数字索引，date列到第1列，保存为str '1991-01-01' 格式
    # 最后调整列顺序
...
def get_tdx_lastestquote(stocklist=None):
    ...
    if api.connect(ucfg.tdx["pytdx_ip"], ucfg.tdx["pytdx_port"]):
        ...
        if len(stocklist_pytdx) == 1:
            ...
            for v in tq:
                ...
                if k > 0 and k % 10 == 0:
                    data = api.to_df(
                        api.get_security_quotes(stocklist_pytdx[k - 10 : k])
                    )
                    df = pd.concat([df, data], axis=0, ignore_index=True)
                elif k == len(stocklist_pytdx) - 1:  # 循环到最后，少于10个构成一组
                    data = api.to_df(
                        api.get_security_quotes(stocklist_pytdx[k - (k % 10) : k + 1])
                    )
                    df = pd.concat([df, data], axis=0, ignore_index=True)
                k = k + 1
        api.disconnect()
    ...
    print(
        f"已获取 {len(df)} 只股票实时行情 用时 {(time.time() - starttime_tick):>.2f} 秒"
    )
...
if __name__ == "__main__":
    stock_code = "600036"
    day2csv(
        ucfg.tdx["tdx_path"] + "/vipdoc/sh/lday",
        "sh" + stock_code + ".day",
        ucfg.tdx["csv_lday"],
    )
    df_gbbq = pd.read_csv(
        ucfg.tdx["csv_gbbq"] + "/gbbq.csv", encoding="gbk", dtype={"code": str}
    )
    df_bfq = pd.read_csv(
        ucfg.tdx["csv_lday"] + os.sep + stock_code + ".csv",
        index_col=None,
        encoding="gbk",
        dtype={"code": str},
    )
